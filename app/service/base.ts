...
export class BaseService extends Service {
    ...
    public async getConnection(name: string | ConnectionOptions | Connection = 'default'): Promise<Connection> {
        ...
        if (typeof name === 'string') {
            ...
            try {
                // @TODO: createConnection 时序问题
                let options = this.config.mysql?.clients.filter(v => {
                    return (v as ConnectionOptions).name === name;
                })[0];
                await createConnection(options as ConnectionOptions);
     ...
     * @param name 连接名称
     */
    public async getConnectionFromManager(name: string): Promise<Connection | null> {
     ...
     */
    public async getRespository<T extends BaseEntity>(
        db: string | Connection = this.dbName,
 ...
 */
export class BaseTypeService<T extends BaseEntity> extends BaseService {
    /**
     * 基础DB服务
     * @param ctx egg  的context
     * @param modelType 当前关联的model
     ...
     * @returns
     */
    public async getNow(db?: string | Connection, type?: ObjectType<T>): Promise<string> {
        const connection = await this.getRespository(db, type);
        const countRes = await connection.createQueryBuilder('default').select('NOW()', 'currentTime').execute();
        let now = countRes[0].currentTime;
        return now || '';
    }
     ...
     * @param db 数据库
     */
    public async save(data: T, db: string | Connection = this.dbName): Promise<T> {
     ...
     * @param db DB
     */
    public async remove(data: T, db: string | Connection = this.dbName): Promise<T> {
     ...
     * @param db DB
     */
    public async recordsCount(tableName: string, data?: any, db: string | Connection = 'default'): Promise<number> {
        let res = await this.getRespository(db);
        return res.createQueryBuilder(tableName).select('COUNT(*)', 'count').where(data).getRawOne();
    }
